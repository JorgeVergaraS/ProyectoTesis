...
export const localAuthInterceptor: HttpInterceptorFn = (request, next) => {
  ...
  const publicRequest = url.pathname.endsWith('/auth/login') || url.pathname.endsWith('/auth/register');
  if (url.origin !== api.origin || !url.pathname.startsWith(api.pathname) || publicRequest) return next(request);
  ...
  return next(outgoing).pipe(catchError((error: unknown) => {
    if (error instanceof HttpErrorResponse && error.status === 401 && !url.pathname.endsWith('/users/me')) {
      store.clear(); void router.navigate(['/login'], { queryParams: { reason: 'expired' } });
    }
